...
class TestUpdate():
    ...
    @pytest.mark.single
    # @pytest.mark.parametrize("user_id, password, mobile, nickname, uuid",
    #                          api_data["test_update_user"])
    # @pytest.mark.usefixtures("update_user_telephone")
    def test_update_user(self, login_fixture, user_id, password, mobile, nickname, uuid):
        logger.info("*************** 开始执行用例 ***************")
        user_info = login_fixture
        access_token = user_info.get("data").get("access_token")
        # update_user(access_token, user_id, password, mobile, nickname)
        # step_1(id)
...
if __name__ == '__main__':
    pytest.main( "test_04_update_user.py")
